...
public class CrittersActor : MonoBehaviour
{
	...
	public event Action<CrittersActor> OnGrabbedChild
	{
		...
		add
		{
			Action<CrittersActor> action = this.OnGrabbedChild;
			Action<CrittersActor> action2;
			do
			{
				action2 = action;
				Action<CrittersActor> action3 = (Action<CrittersActor>)Delegate.Combine(action2, value);
				action = Interlocked.CompareExchange<Action<CrittersActor>>(ref this.OnGrabbedChild, action3, action2);
			}
	...
	[CompilerGenerated]
	private Action<CrittersActor> OnGrabbedChild;
